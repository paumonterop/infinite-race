...
import { getVmixTally, parseProgramInputs } from "./vmixClient";
...
let lastVmixCheck = 0;
let vmixCheckInFlight = false;

/** Called periodically (server tick) - if vMix auto-selection is enabled,
 * asks vMix (via its TCP API) which input is currently in Program and,
 * using the configured GPS↔input mapping, updates gps_selected_ids so the
 * broadcast profile scene automatically follows whichever camera/GPS is on
 * air. Throttled to avoid opening a TCP connection every single tick. */
export async function checkVmixAutoSelect() {
  const race = getRace();
  if (!race.vmix_auto_select || !race.vmix_host) return;
  if (vmixCheckInFlight) return;
  const now = Date.now();
  if (now - lastVmixCheck < 3000) return;
  lastVmixCheck = now;
  vmixCheckInFlight = true;
  try {
    const tally = await getVmixTally(race.vmix_host, race.vmix_port || 8099);
    const programInputs = parseProgramInputs(tally);
    const map = race.vmix_gps_map || {};
    const gpsIds = Object.entries(map)
      .filter(([, input]) => programInputs.includes(Number(input)))
      .map(([gpsId]) => gpsId);

    if (gpsIds.length === 0) return; // no mapped input in program right now; leave selection as-is

    const store = getStore();
    const current = store.race.gps_selected_ids || [];
    const changed = gpsIds.length !== current.length || gpsIds.some((id) => !current.includes(id));
    if (changed) {
      store.race = { ...store.race, gps_selected_ids: gpsIds, updated_at: new Date().toISOString() };
      persist();
      emitUpdate("race:changed");
    }
  } catch {
    // vMix pot estar apagat o inaccessible; ho tornarem a provar al proper tick, sense fer soroll.
  } finally {
    vmixCheckInFlight = false;
  }
}
...
/** Called periodically (server tick) - if a scheduled start is enabled and the
 * programmed time has arrived, starts the race automatically. Only fires while
 * the race hasn't started yet (SETUP/READY), so it never interferes with a
 * race already running, paused or finished. */
export function checkScheduledStart() {
  const race = getRace();
  if (!race.auto_start_enabled || !race.scheduled_start_at) return;
  if (race.status !== "SETUP" && race.status !== "READY") return;
  const store = getStore();
  if (store.runners.length === 0) return; // nothing to start yet
  const startsAt = new Date(race.scheduled_start_at).getTime();
  if (Date.now() >= startsAt) {
    // One-shot: disable it after firing so a later RESET doesn't cause the
    // race to restart itself again immediately (scheduled time would still
    // be in the past).
    const s = getStore();
    s.race = { ...s.race, auto_start_enabled: false };
    logEvent("RACE_AUTO_START", { message: "Inici automàtic programat disparat" });
    startRace();
  }
}
